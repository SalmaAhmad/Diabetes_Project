...
# KNN Classifier - Find Best k using Test Accuracy Graph
# Encode target labels
label_encoder = LabelEncoder()
y_train_enc = label_encoder.fit_transform(y_train)
y_test_enc = label_encoder.transform(y_test)

print("Finding best k using k-fold cross-validation...\n")
...
for k in k_values:
    ...
    scores = cross_val_score(knn, X_train, y_train_enc, cv=kf, scoring='accuracy')
    cv_scores.append(scores.mean())
    print(f"k = {k:2d} → Accuracy = {scores.mean():.4f}")
...
try:
    ...
    for idx, class_name in enumerate(classes):
        ...
        print(f"\nClass {class_name}:")
        print(
            f"  Specificity: {specificity:.4f} - Correctly identified {specificity * 100:.1f}% of non-{class_name} cases")
